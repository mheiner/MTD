...
struct TransTens
    ...
    function TransTens(A::Array{T}) where T <: Real
        ...
        maximum(abs.( sum(A, dims=1) .- 1.0 )) < 1.0e-6 || error("Transition tensor must add to unity along first dimension.")
        return new(A, order)
    end
end
...
function reduce_intercept(Omeg::TransTens)
    ...
    maximum(abs.( leftover_sums .+ lam .- 1.0 )) < 1.0e-6 || error("All leftover marginal sums should be equal.")
